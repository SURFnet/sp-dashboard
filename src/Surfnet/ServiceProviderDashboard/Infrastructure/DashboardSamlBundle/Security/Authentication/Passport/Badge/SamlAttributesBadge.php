...
namespace Surfnet\ServiceProviderDashboard\Infrastructure\DashboardSamlBundle\Security\Authentication\Passport\Badge;
        ...
        foreach ($attributes as $key => $attributeParts) {
            foreach ($attributeParts as $index => $attribute) {
                ...
                if ($attribute instanceof NameID) {
                    $attributes[$key][$index] = ['Value' => $attribute->value, 'Format' => $attribute->Format];
                }
            }
        }
