...
const MANAGE_ENTITY_SEARCH_RETRIES = 30;
const MANAGE_ENTITY_SEARCH_WAIT_MS = 1000;
...
function findManageEntityByEntityId(entityType, entityId, tries = 0) {
    return cy.request({
        method: 'POST',
        url: `${MANAGE_URL}/manage/api/internal/search/${entityType}`,
        headers: MANAGE_HEADERS,
        ...
        if (tries >= MANAGE_ENTITY_SEARCH_RETRIES) {
            throw new Error(`Manage entity not found for ${entityType}:${entityId}`);
        }

        cy.wait(MANAGE_ENTITY_SEARCH_WAIT_MS);

        return findManageEntityByEntityId(entityType, entityId, tries + 1);
    });
}

Cypress.Commands.add('waitForManageEntity', (entityType = 'saml20_sp', entityId = 'https://tiffany.aching.do/id') => {
    return findManageEntityByEntityId(entityType, entityId);
});
...
Cypress.Commands.add('loginToManageAndSelectTiffanyAching', (url = MANAGE_URL, entityId = 'https://tiffany.aching.do/id', entityType = 'saml20_sp') => {
    cy.loginToManage(url);
    cy.waitForManageEntity(entityType, entityId).then((entity) => {
        cy.visit(`${url}/metadata/${entityType}/${entity._id}/connection`);
    });
});
